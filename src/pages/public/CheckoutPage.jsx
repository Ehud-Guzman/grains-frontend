...
import { ArrowLeft, ArrowRight, Check, MapPin, CreditCard, User, ShoppingBag, Truck, Store } from 'lucide-react'
...
// ── OPTION CARD ───────────────────────────────────────────────────────────────
const OptionCard = ({ icon: Icon, label, desc, checked, onChange }) => (
  <label className={`flex items-start gap-4 p-4 border-2 rounded-xl cursor-pointer transition-all ${
    checked ? 'border-brand-500 bg-brand-50 shadow-sm' : 'border-earth-200 hover:border-earth-300 bg-white'
  }`}>
    ...
    <div className="flex-1">
      <p className={`font-body font-semibold text-sm ${checked ? 'text-brand-800' : 'text-earth-800'}`}>{label}</p>
...
export default function CheckoutPage() {
    ...
    return (
      <div className="min-h-screen bg-cream flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-earth-500 text-sm font-body mb-6">Add some products before checking out</p>
  ...
  const submit = async () => {
    ...
    try {
      ...
      clearCart()
      // Pass via state AND query param for maximum compatibility
      const orderRef = res.data.data.orderRef
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <div className="bg-earth-900 text-cream py-6">
        <div className="container-page max-w-3xl">
          <div className="flex items-center gap-3 mb-4">
            <Link to="/cart" className="text-earth-400 hover:text-cream transition-colors">
              <ArrowLeft size={18} />
            </Link>
                ...
                <div key={s.label} className="flex items-center flex-1 last:flex-none">
                  ...
                  >
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs
                      font-semibold border-2 transition-all flex-shrink-0 ${
                  ...
                  {i < STEPS.length - 1 && (
                    <div className="flex-1 h-px mx-2 bg-earth-700">
                      <div className={`h-full bg-brand-500 transition-all duration-500 ${done ? 'w-full' : 'w-0'}`} />
                    </div>
                  )}
                </div>
              )
      ...
      <div className="container-page max-w-3xl py-6 pb-12">
            ...
            <div className="bg-white rounded-2xl border border-earth-100 shadow-sm p-6">
              ...
              {step === 0 && (
                <div className="space-y-5">
                  <div>
                    <h2 className="font-display text-xl text-earth-900 font-semibold">Contact Details</h2>
                    <p className="text-earth-500 text-sm font-body mt-1">We'll use this to confirm your order</p>
                  </div>
              ...
              {step === 1 && (
                <div className="space-y-5">
                  <div>
                    <h2 className="font-display text-xl text-earth-900 font-semibold">Delivery Method</h2>
                    <p className="text-earth-500 text-sm font-body mt-1">How would you like to receive your order?</p>
                  </div>
                  ...
                  <Field label="Special Instructions">
                    <textarea
                      ...
                      className="w-full border border-earth-200 rounded-xl px-4 py-3 text-sm font-body
                        ...
                        focus:ring-brand-400 focus:border-transparent transition-all bg-earth-50 resize-none"
                    />
                  </Field>
                </div>
              )}
              ...
              {step === 2 && (
                <div className="space-y-5">
                  <div>
                    <h2 className="font-display text-xl text-earth-900 font-semibold">Payment Method</h2>
                    <p className="text-earth-500 text-sm font-body mt-1">How would you like to pay?</p>
                  </div>
                  <div className="space-y-3">
              ...
              {step === 3 && (
                <div className="space-y-5">
                  <div>
                    <h2 className="font-display text-xl text-earth-900 font-semibold">Review Order</h2>
                    <p className="text-earth-500 text-sm font-body mt-1">Check everything looks right before confirming</p>
                  </div>

                  {[
                    {
                      label: 'Contact', step: 0,
                      content: <p className="text-sm text-earth-700 font-body">{form.name} · {form.phone}</p>
                    },
                    {
                      ...
                      content: (
                        <>
                          ...
                          {form.deliveryAddress && (
                            <p className="text-xs text-earth-500 mt-0.5 font-body">{form.deliveryAddress}</p>
                          )}
                        </>
                      )
                    },
                    {
                      ...
                      content: <p className="text-sm text-earth-700 font-body">{PAYMENT_LABELS[form.paymentMethod]}</p>
                    },
                  ].map(row => (
                    <div key={row.label} className="bg-earth-50 rounded-xl p-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-body font-semibold text-earth-500 uppercase tracking-wide">
                          {row.label}
                        </span>
                      ...
                      {items.map(item => (
                        <div key={item.key} className="flex justify-between items-start text-sm">
                          <div className="min-w-0 pr-4">
                            <span className="text-earth-800 font-body font-medium">{item.productName}</span>
                            <span className="text-earth-500 font-body"> · {item.variety} · {item.packaging} ×{item.quantity}</span>
                          </div>
              ...
              {step === 4 && (
                <div className="text-center py-6 space-y-4">
                  ...
                  <div className="bg-earth-50 rounded-xl p-4 text-left space-y-2">
                    ...
                    <div className="flex justify-between text-sm font-body border-t border-earth-100 pt-2 mt-2">
                      <span className="font-semibold text-earth-800">Total</span>
                      <span className="font-display font-bold text-brand-600 text-lg">{formatKES(total)}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="flex justify-between mt-6 pt-5 border-t border-earth-100">
                {step > 0 ? (
                  <button onClick={back}
                    className="flex items-center gap-1.5 px-4 py-2.5 text-sm font-body text-earth-600
                      hover:text-earth-900 hover:bg-earth-100 rounded-xl transition-colors">
                ...
                {step < 3 && (
                  <button onClick={next}
                    className="flex items-center gap-2 px-6 py-2.5 bg-earth-900 text-white rounded-xl
                      text-sm font-body font-semibold hover:bg-earth-800 transition-all active:scale-[0.98]">
                ...
                {step === 4 && (
                  <button onClick={submit} disabled={loading}
                    className="flex items-center gap-2 px-8 py-3 bg-brand-500 text-white rounded-xl
                      text-sm font-body font-bold hover:bg-brand-600 transition-all active:scale-[0.98]
                      disabled:opacity-60 disabled:cursor-not-allowed shadow-warm">
                    {loading ? (
                      <>
                ...
                {items.map(item => (
                  <div key={item.key} className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-body font-medium text-earth-800 truncate">{item.productName}</p>
                      <p className="text-xs text-earth-500 font-body">{item.variety} · {item.packaging}</p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="text-xs font-body font-semibold text-earth-800">{formatKES(item.priceKES * item.quantity)}</p>
