...
import { ArrowLeft, Phone, XCircle, Package, Truck, Store, CreditCard } from 'lucide-react'
    ...
    <div className="min-h-screen bg-cream">
      ...
      <div className="bg-earth-900 pt-8 pb-16 px-4">
        <div className="container-page max-w-2xl">
          ...
          <h1 className="font-display text-2xl font-bold text-cream mb-1">{order.orderRef}</h1>
      ...
      <div className="container-page max-w-2xl -mt-8 pb-12 space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-warm-lg border border-earth-100 overflow-hidden">
                    ...
                    <div key={s} className="flex items-center flex-1 last:flex-none">
                      <div className="flex flex-col items-center">
                        <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center
                          text-xs font-bold font-body transition-all ${
                      ...
                      {i < STATUS_STEPS.length - 1 && (
                        <div className={`flex-1 h-0.5 mx-1 mb-5 ${done ? 'bg-brand-500' : 'bg-earth-100'}`} />
                      )}
                    </div>
                  )
                })}
          ...
          {order.status === 'rejected' && order.rejectionReason && (
            <div className="mx-5 mb-4 bg-red-50 border border-red-200 rounded-xl p-4 text-sm font-body text-red-700">
        ...
        <div className="bg-white rounded-2xl border border-earth-100 shadow-warm overflow-hidden">
          <div className="px-5 py-4 border-b border-earth-50">
            <p className="text-xs font-body font-semibold text-earth-500 uppercase tracking-wide">Items Ordered</p>
          </div>
          <div className="divide-y divide-earth-50">
            {order.orderItems?.map((item, i) => (
              <div key={i} className="flex items-center justify-between px-5 py-4">
                <div className="flex-1 min-w-0 pr-4">
                  <p className="font-body font-semibold text-earth-800 text-sm">{item.productName}</p>
            ...
            <div className="flex items-center justify-between px-5 py-4 bg-earth-50">
              ...
              <span className="font-display font-bold text-brand-600 text-lg">{formatKES(order.total)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <a href={`tel:${SHOP_INFO.phone}`}
            className="flex items-center justify-center gap-2 w-full py-3.5 bg-earth-900
              text-cream rounded-xl text-sm font-body font-semibold hover:bg-earth-800 transition-colors">
